...
* AIの出力過程で、LaTeXコマンドのバックスラッシュ（\）がエスケープ文字として消費され、消えてしまうエラー（例: \mathbb{C} が mathbb{C} になる）を厳密に防いでください。
コードブロックの出力結果において、必ず「\」が文字として残るように、AI側の内部処理で適切にエスケープ（例：\\ と出力する等）を行ってください。

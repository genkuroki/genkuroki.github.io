...
\begin{align}
 ...
 \notag 
 \\ &
  s_i(q^{\epsv_i}) = q^{\epsv_{i+1}}, \quad
  s_i(q^{\epsv_{i+1}}) = q^{\epsv_i}, \quad
  s_i(q^{\epsv_j}) = q^{\epsv_j} \quad (j\ne i,i+1),
 \label{eq:s_i(t_j)}
 \\ &
  s_i(\tau_i) 
...
For example, we have $p_{i,i+1} = - g_i^{-1} = qf_i/[\av_i]_q$. 
...
The actions of $s_i$ on $z_j$'s are explicitly written as
\begin{equation*}
...
\begin{align*}
 &
 \hs_i(z_i) = [\av_i]_q^{-1}f_i z_{i+1} [\av_i]_q^{-1}, \quad
 \hs_i(z_{i+1}) = [\av_i]_q f_i^{-1} z_i [\av_i]_q, 
 \\ &
 \hs_i(z_j) = z_j \quad (j\ne i,i+1).
\end{align*}
...
\begin{proof}
 It immediately follow from \eqref{eq:s_i(t_j)} 
 that $s_i(D_t)=S_i D_t S_i^{-1}$.
 Recall that we have $M=q^{-2}Z D_t^2 Z^{-1}$.
 Put $Z_i = G_i Z S_i$ and $D_{t,i} = S_i^{-1}D_t S_i$. 
 Then we have
 \begin{equation*}
  q^{-2} Z_i D_{t,i}^2 Z_i^{-1} 
  = G_i M G_i^{-1} 
  = s_i(M) 
  = \hs_i(M).
 \end{equation*}
 Moreover the $j$-th diagonal entry of $Z_i$ coincides with $\hs_i(z_j)$.
 Therefore the theorem follows from \theoremref{theorem:Lax-A_{n-1}}. 
 \qed
\end{proof}

...
\def\DATE{May 27, 2012, \quad Version 0.04\thanks{
...
\label{prop:QWGA-A_{n-1}}
 ...
 the algebra automorphism $\bs_i$ of $D(K^\pa)$ can be given by
 \begin{align*}
  &
  \bs_i(f_j)
...
the quantum birational Weyl group action in \propref{prop:QWGA-A_{n-1}} by
\begin{equation*}
 s_i(x) = f_i^{\av_i} \ts_i(x) f_i^{-\av_i}
 \quad (i=1,2,\ldots,n-1; x\in D(K^\pa)),
\end{equation*}
where $f_i^{\av_i}$ denotes the fractional power of $f_i$
defined in \cite{Kuroki2012a}.
\qed
\end{remark}
...
\label{theorem:Lax-A_{n-1}}
 ...
 on $K^\pa=\C(q,f_i,t_k|i=1,2,\ldots,n-1;k=1,2,\ldots,n)$.
\end{theorem}
...
\begin{remark}[Quantum unipotent crystal structure on $M$]
 In this remark, we shall generalize the algebra automorphisms 
 $\bs_i$ ($i=1,2,\ldots,n-1$) and 
 constructs a quantum unipotent crystal structure on $M$.
 ...
 \end{equation*}
 Equivalently, we have $\be_i^c(q^\beta)=c^{\bra\beta,\alpha_i\ket}q^\beta$
 for $\beta\in\Qv$.
 Then we have $\be_i^c = \bs_i$ if $c = q^{-\av_i}$.
 Setting $\alpha_i(M) = m_{ii}/m_{i+1.i+1}$, 

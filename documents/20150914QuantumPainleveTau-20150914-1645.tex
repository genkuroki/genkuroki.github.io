...
量子化された $\tau$ 変数 \quad {\red $\tau_i = \exp(\d/\d\av_i)$} \quad (差分作用素):
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame{
\frametitle{middle convolution}
\begin{footnotesize}

$D^{(1)}_4$ 型の場合で説明: \vspace{-\medskipamount}
\[
 f_2 = \d_x = \frac{\d}{\d x}, \qquad
 f_i = x - t_i \quad (i=0,1,3,4)
\]
$i=0,1,3,4$ について, \quad $[f_2,f_i]=1$ \quad なので
\[
 s_2(f_i) = f_i + \av_2{f_2}^{-1}, \qquad
 s_2(\tau_2) = f_2\,\frac{\tau_0\tau_1\tau_3\tau_4}{\tau_2}.
\]
すなわち
\[
 s_2(x) = x - \av_2{\d_x}^{-1}, \qquad
 s_2(\tau_2) = \d_x\,\frac{\tau_0\tau_1\tau_3\tau_4}{\tau_2}.
\]

${f_2}^\gamma={\d_x}^\gamma$ のEuler変換による実現:
\[
 {\d_x}^\gamma f(x)
= {}_aD_x^\gamma f(x)
= \frac{1}{\Gamma(-\gamma)}
\int_a^x (x-y)^{-\gamma-1}f(y)\,dy.
\]

\medskip

注意: $f_i$, $f_j$ のどちらかが $1$ ならば, Vema関係式は常に成立するので, 
欠けてしまって足りなくなった分の $f_i$ は $1$ として補充可能.
...
Schlesinger 系(1階連立の場合)の量子化 \\
$=$ Knizhnik-Zamolodchikov方程式 (WZW model)

\bigskip\bigskip

Garnier 系(2階単独の場合)の量子化 \\
...
\frametitle{}
\begin{small}
補足: $L(z)=L_1(z)L_2(z)$ の対角部分 $L_0$ を二重化した \\
$\tL(z)=L(z)L_0$ を考える理由は以下のLax表示の存在.

\bigskip\bigskip

$f_i:=\dfrac{\text{($L_0^{-1}\tL(z)L_0^{-1}$ の $(i,i+1)$ 成分)}}{q^{-1}-q}
=(q^{-1}-q)^{-1}\ta_i^{-1}b_i\ta_{i+1}^{-1}$.

\medskip

$f_i$ たちは $q$-Serre 関係式をみたしている.

\bigskip

$\cG_i:=E+(c^2-1)\ta_{i+1}^2 b_i^{-1}E_{i+1,i}$, \\
$\cG'_i:=E+(c^{-2}-1)b_i^{-1}\ta_i^2E_{i+1,i}$, \quad 
$c:=q^{-\gamma}$ \quad とおくと
\[
  {\red f_i^\gamma \tL(z) f_i^{-\gamma} = \cG_i \tL(z) \cG'_i}.
\]
$\tL(z)$ は{\magenta 量子化された幾何クリスタル}ともみなせる.
...
$\tW(A^{(1)}_2)\times\tW(A^{(1)}_1)$ の作用のLax表示

\bigskip

$i=1,2$ に対して, 
$g_i=(t_i^2-t_{i+1}^2)/(x_i+y_{i+1})$, $G_i=E+g_i E_{i+1,i}$, 
$G'_i=\varpi(G_i)$ とおく. $\Lambda(z)=E_{12}+E_{23}+zE_{31}$ とおく.
このとき,
\begin{align*}
&
s_i(X(z)) = G_i X(z) {G'_i}^{-1}, \quad
\\ &
s_i(Y(z)) = G'_i Y(z) G_i^{-1}, 
\\ &
\pi(X(z)) = \Lambda(z)X(z)\Lambda(z)^{-1}
\\[2\medskipamount] &
r_1(X(z)Y(rz))=X(z)Y(rz), \quad
\\ &
r_1: x_{i+2}x_{i+1}x_i \leftrightarrow y_{i+3}y_{i+2}y_{i+1}, 
\\ &
\varpi: X(z) \leftrightarrow Y(z).
\end{align*}
これらの条件で作用が一意に特徴付けられる.
...
(3) $\tL(z)=L(z)L_0$ ← {\blue 対角部分 $L_0$ を二重化}

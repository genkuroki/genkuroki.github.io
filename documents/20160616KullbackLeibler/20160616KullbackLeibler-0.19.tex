...
その点での $n D(p||q)$ をTaylor展開した結果の2次の部分から
件の多次元正規分布の指数函数部分の対数の $-1$ 倍が得られる.
そのことを確認しよう. $\sum_{i=1}^r x_i=0$ と制限せずに $nD(p||q)$ 
...
\end{align*}
このことから正規分布での近似は $p_i$ たちが $q_i$ たちに近いときに

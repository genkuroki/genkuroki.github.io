...
他にも細かな訂正. 
相対Renyiエントロピーの定義だけに触れた\remarkref{remark:Renyi-Free}を追加した.
}
\\[\bigskipamount]
{\small
...
\]
から得られる自由エネルギー $F(\beta)=-\beta^{-1}\log Z(\beta)$ の定義
と本質的に同じである:
\[
-(\beta-1)S_\beta[p|q] = -\beta F(\beta) = \log Z(\beta).
\]
...
確率 $\theta$ に対応する二項分布は確率 $1/2$ の二項分布を母集団分布とする場合の
Gibbs分布として現われることを説明しよう. この例は母集団分布が一様分布でない
場合のGibbs分布の簡単な例になっている.

...
6月20日Ver.0.5(26頁). \remarkref{remark:beta}に
...
こちらの $\mathcal F$ は{\bf Massieu函数}と呼ばれているらしい.
\qed
\end{remark}
...
\]
に付随する自由エネルギー $F(\beta)$ とMassieu函数 ${\mathcal F}(\beta)$ の定義
\[
F(\beta)=-\beta^{-1}\log Z(\beta), \qquad
{\mathcal F}(\beta)=\log Z(\beta) \quad (\text{Boltzmann定数倍は略})
\]
と本質的に同じである:
\[
(\beta-1)S_\beta[p|q] = \beta F(\beta) = -{\mathcal F}(\beta) = -\log Z(\beta).
\]
R\'enyi divergence (相対R\'enyiエントロピーの $-1$ 倍)の基本性質の
まとめが \cite{vanErven-Harremoes} にある.

$(\beta-1)S_\beta[p|q]=-\log Z(\beta)$ は $\beta$ の函数として上に凸である:
\begin{align*}
&
...
\leqq 0
\\ &
(\text{等号成立と $p_i=q_i$ ($i=1,\ldots,r$) は同値}).
\end{align*}
そして, $(\beta-1)S_\beta[p|q]=-\log Z(\beta)$ の 
$\beta=1$ での値が $-\log Z(1)=-\log 1=0$ であることと,
$(\beta-1)S_\beta[p|q]=-\log Z(\beta)$ の $\beta=1$ での微係数
...
\begin{remark}[Tsallisエントロピー]
\label{remark:Tsallis}
$p_i,q_i$ は確率分布であるとし, $Z(\beta)$ を次のように定める:
\[
Z(\beta) = \sum_{i=1}^r \left(\frac{p_i}{q_i}\right)^\beta q_i.
\]
このとき
\[
-\frac{1}{\beta-1}\log Z(\beta)=S_\beta[p|q]=(\text{相対R\'enyiエントロピー})
\]
であり, 
\[
-\left.\frac{\d}{\d\beta}\right|_{\beta=1} Z(\beta) 
= -\sum_{i=1}^r p_i\log\frac{p_i}{q_i}
=S[p|q]
=(\text{相対エントロピー}).
\]
相対エントロピーの式の微分を $q$ 差分で置き換えることによって
得られる量は{\bf 相対Tsallisエントロピー}と呼ばれている:
\[
S^{(q)}[p|q] 
= -\frac{Z(1)-Z(q)}{1-q}
= -\frac{1-\sum_{i=1}^r p_i^q q_i^{1-q}}{1-q}.
\]
$q\to 1$ で相対Tsallisエントロピーは相対エントロピーに収束する.
\qed
\end{remark}
...
\]
この場合については\secref{sec:normal-Gibbs}も参照して欲しい.
...
\end{align*}
ゆえに, Kullback-Leibler情報量が常に $0$ 以上であることと不等式($\#$)より, 
\[
D[p|q]
\geqq D[p|p(\beta)] -\beta U(\beta)-\log Z(\beta) 
\geqq D[p|p(\beta)] + D[p(\beta)|q]
\geqq D[p(\beta)|q].
\]
...
\]
を意味する. 田崎 \cite{Tasaki}の第9-2-1節(p.319)も参照せよ.

Gibbs分布が経験的に自然に得られることを示すためには, 
...
そして $\beta>0$ を大きくすると $U(\beta)$ は $\min\{E_1,\ldots,E_r\}$ に近付き, 
$\beta<0$ を小さくすると $U(\beta)$ は $\max\{E_1,\ldots,E_r\}$ に近付く.
...
温度の高さを $\beta$ の小ささで測ることにすれば, 負の絶対温度は
...
ゲーム自体の期待値である $U_0$ より大きい $U$ 以上になるようにする.
このとき, $n$ 回のゲーム中 $i$ の目が出た割合 $p_i$ は
($n$ が大きなとき)どのような値になるか?

ゲーム1回あたりの賞金の平均値が $U_0$ から離れれば離れるほど
そのような状況が生じる確率は下がるので, 
ゲーム1回あたりの賞金の平均値はほぼ $U$ になってしまうだろう.

そのときの $i$ の目が出た割合 $p_i$ を計算すると, 
$U$ に対応する $\beta$ に対応するGibbs分布 $p_i(\beta)$ に
近くなるというのがこの節において数学的にきちんと証明したことである.
...
そのような状況が生じる確率は下がるので, 
ゲーム1回あたりの賞金の平均値はほぼ $U$ になり, 
$i$ の目が出た割合は $U$ に対応するGibbs分布 $p_i(\beta)$ に近くなるだろう.
...
以上の説明を読めばGibbs分布の導出で使った不等式 ($\#$) の向きが
どのように自然であるかがわかると思う.
...
確率 $1/2$ の二項分布を母集団分布とする場合のGibbs分布として
自然に現われることを説明しよう. 
この例は母集団分布が一様分布でない場合のGibbs分布の簡単な例になっている.
...
\tag{$*$}
\]
このとき $p_i=k_i/n$ たちはどのような値になっているだろうか?

「経験分布 $p=(p_0,p_1,\ldots,p_r)$ は確率 $\theta$ 
に対応する二項分布にほぼ等しくなっているだろう」というのが, 
\theoremref{theorem:Gibbs}をすぐ上の計算に適用した結論になる.

上で計算したGibbs分布 $p(\beta)$ 
...
そのリストにはあたかも表の出る確率が $\theta$ であるかのような二項分布の
記録が残っていることになる.
\qed
\end{example}
...
\begin{remark}
\exampleref{example:binom-Gibbs}の議論を一般化すると以下が成立すること
がわかる.
母集団分布 $q_i$ はGibbs分布の形をしていると仮定する:
\[
...
$q_1,q_r>0$ であると仮定する.
このとき, この母集団分布に対応するGibbs分布 $p(\beta)$ も
母集団分布と同じ形のGibbs分布になる:
\[

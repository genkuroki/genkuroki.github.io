...
\[
\lim_{n\to\infty}
P_\beta\left(\frac{1}{n}\sum_{k=1}^n H_k \in (u-\delta,u+\delta)\right)
=1.
\]
そして, カノニカル分布ともとの確率分布での確率のあいだに以下の関係がある:
\begin{align*}
P_\beta\left(\frac{1}{n}\sum_{k=1}^n H_k \in (u-\delta,u+\delta)\right)
&
=\frac
{E[1_{H_1+\cdots+H_n\in(nu-n\delta,nu+n\delta)}\,e^{-\beta(H_1+\cdots+H_n)}]}
{Z(\beta)^n}
\\ &
\leqq
Z(\beta)^{-n}
E[1_{H_1+\cdots+H_n\in(nu-n\delta,nu+n\delta)}\, e^{-n\beta u + n|\beta|\delta}]
\\ &
...
P\left(\frac{1}{n}\sum_{k=1}^n H_k \in (u-\delta,u+\delta)\right)
\geqq
e^{n(\beta u+\log Z(\beta)+|\beta|\delta)}(1+o(1))
\qquad (n\to\infty).
\end{align*}
ゆえに両辺の対数の $1/n$ の極限を取ることによって次を得る:
\begin{align*}
\liminf_{n\to\infty} \frac{1}{n}
\log P\left(\frac{1}{n}\sum_{k=1}^n H_k \in (u-\delta,u+\delta)\right)
\geqq
\beta u + \log Z(\beta) =S(u).
\end{align*}
これで(2)も示された.
\qed
\end{proof}
...
\subsection{相対エントロピーとの関係}

$q=(q_1,\ldots,q_r)$ は有限集合 $\{1,\ldots,r\}$ 上の
確率分布であるとする: $q_i\geqq 0$, $q_1+\cdots+q_r=1$.
確率変数 $H$ を $H(i)=E_i$ と定める.
このとき
\[
...
$H_1,H_2,\ldots$ が独立同分布な確率変数列で各々が $H$ と同じ分布に
したがうとき, $H_1+\cdots+H_n$ は形状 $n\alpha$, スケール $\tau$ の
ガンマ分布にしたがうので,
\begin{align*}
...
\end{align*}
Cram\'erの定理より, これの対数の $1/n$ 倍の $n\to\infty$ での極限は
\begin{align*}
\sup_{a<u<b}S(u)=
\begin{cases}
S(b) & (b<U(0)=\tau\alpha), \\
0    & (a\leqq U(0)=\tau\alpha\leqq b), \\
S(a) & (U(0)=\tau\alpha<a)
\end{cases}
\end{align*}
に収束する.

...
の構成に関する\secref{sec:Tsallis-multinomial}を追加した.
}
\\[\bigskipamount]
{\small
...
この解説ノートを書いたもう一つの理由は, 
Boltzmann因子, Gibbs分布(カノニカル分布)が出て来る理由を
...
\subsection{自由エネルギーやMassieu函数や他の種類のエントロピーとの関係}
\label{sec:entropies}
...
Gibbs分布 $p(\beta)=(p_1(\beta),\ldots,p_r(\beta))$ を
\[
p_i(\beta)
...
\subsection{相対Tsallisエントロピーを漸近挙動に含む多項分布の拡張の構成}
\label{sec:Tsallis-multinomial}
...
この記号法を使うと, 
\secref{sec:entropies}で導入した相対Tsallisエントロピー $T_\beta[p|q]$ を
次のように表わせる%
...
$U$ が定める $\beta$ に対応するGibbs分布 $p_i(\beta)$ に
...
$i$ の目が出た割合は $U$ に対応するGibbs分布 $p_i(\beta)$ に近くなる可能性が高い.
...
母集団分布 $q_i$ は最初から $E_i$ に関するGibbs分布の形をしていると仮定する:
\[
...
$q_1,q_r>0$ であると仮定する.
このとき, この母集団分布と $E_i$ たちに対応するGibbs分布 $p(\beta)$ も

...
{\tt http://www.math.tohoku.ac.jp/{\textasciitilde}kuroki/LaTeX/20160616KullbackLeibler/}
}
\end{center}
Ver.0.1は10ページしかなかった.
...
{\tt https://twitter.com/genkuroki/status/773390919450132481}
\end{center}
の内容をまとめ直したものである.
...
{\tt https://twitter.com/genkuroki/status/775549559405555713}
\end{center}
...
+O\left(\frac{1}{\lambda\sqrt{\lambda}}\right)
=-\frac{x^2}{2}
...
そのとき $w=C(s)=w_{i_1}\cdots w_{i_n}$ は $b$ 個の文字を $k$ 個並べて
...
この不等式は次のように書き直される:
\begin{align*}
a_i\geqq 0,\; \sum_{i=1}^t a_i\leqq 1 \implies
...
このとき, 対数の底が $b$ のKullback-Leibler情報量の $q$ を $a$ に拡張したもの
\[
D_b(p||a)
=\sum_{i=1}^r p_i\log\left(\frac{p_i}{a_i}\right)
=\sum_{i=1}^r p_i(-\log_b a_i) - \sum_{i=1}^r p_i(-\log p_i)
\geqq 0
\]
は確率分布 $a_i$ に対応する方法で符号化するときに
「エントロピーよりも無駄に増えた平均符号長の大きさ」を意味していると解釈される.

このようにKullback-Leibler情報量には「平均符号長のエントロピーからの増分」
という解釈が可能である.
